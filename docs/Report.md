...
2. **Subscriber Workflow** — individual retention tracker with four states (AI Scored → Reviewed → Contacted → Outcome). One click generates a Claude-authored brief per subscriber: a 2–3 sentence risk narrative citing the decisive features, a recommended action (channel, offer, urgency, rationale), and a personalized email + Twilio voice script. The reviewer can approve / escalate / mark-safe the prediction, override the channel, **edit the email subject, body, and voice script before send**, capture free-text reviewer notes, record the outcome (Retained / Churned / No response), and submit the full row through a **Save feedback** button that POSTs to `/api/feedback` — closing the retraining loop and locking the row from further edits.
...
The single-screen "value moment" the prototype is designed to deliver is the **Churn Radar → Subscriber Workflow** flow. A CVM analyst opens Churn Radar, sees the top-N highest-risk subscribers ranked by predicted probability — the dashboard surfaces a concrete number on this screen ("Projected MRR loss if no action: **$1,247**" against the six most-at-risk subscribers in the current synthetic dataset). The analyst opens one subscriber (Subscriber Workflow), clicks *Generate AI brief* — Claude returns a 2–3 sentence risk narrative, a recommended channel + offer + urgency, and a fully drafted email and voice script personalized to that subscriber's plan, city, and pain points. The analyst can approve, escalate, or override the AI prediction; override the recommended channel; **edit the email subject, body, or voice script before send**; dispatch through Gmail SMTP or Twilio Voice; and record the outcome (Retained / Churned / No response) plus reviewer notes — all from one tab, in under a minute. The same flow today involves three teams and two days.
